...
class ANTScoringDemo {
    private helper: ANTScoringHelper;
    private accounts: {
        deployer: AptosAccount;
        researchers: AptosAccount[];
        scorers: AptosAccount[];
    };
    ...
    constructor() {
        ...
        this.helper = new ANTScoringHelper(DEMO_CONFIG, this.accounts.deployer);
    }
     ...
     */
    private printProposal(id: number, title: string, researcher: string, description: string) {
     ...
     */
    private printScoring(scorer: string, scores: ScoreInput, finalScore: number) {
     ...
     */
    async simulateProposalSubmission(): Promise<number[]> {
    ...
    async simulateScoring(proposalIds: number[]): Promise<number[]> {
        ...
        for (let proposalIndex = 0; proposalIndex < proposalIds.length; proposalIndex++) {
            ...
            console.log(`${colors.bright}Individual Scores: ${finalScores.slice(-1)[0] === averageScore ? 
                scoringScenarios[proposalIndex].scores.map((_, i) => 
                    this.helper.calculateFinalScore(scoringScenarios[proposalIndex].scores[i])
                ).join('%, ') + '%' : ''
            }${colors.reset}`);
     ...
     */
    async simulateFulfillment(proposalIds: number[], finalScores: number[]): Promise<void> {
     ...
     */
    async displaySummary(proposalIds: number[], finalScores: number[]): Promise<void> {
