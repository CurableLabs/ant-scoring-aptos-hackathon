...
export class APIGateway {
    ...
    constructor(antHelper: ANTScoringHelper, port: number = 3001) {
        this.app = express();
        this.antHelper = antHelper;
        this.port = port;
        this.eventProcessor = new EventProcessor(antHelper);
    ...
    private async getAllProposals(req: express.Request, res: express.Response): Promise<void> {
        try {
            const activeIds = await this.antHelper.getActiveProposals();
            const proposals = await Promise.all(
                activeIds.map(id => this.antHelper.getProposal(id))
            );
    ...
    private async getProposal(req: express.Request, res: express.Response): Promise<void> {
        try {
    ...
    private async createProposal(req: express.Request, res: express.Response): Promise<void> {
        try {
    ...
    private async scoreProposal(req: express.Request, res: express.Response): Promise<void> {
        try {
    ...
    private async fulfillProposal(req: express.Request, res: express.Response): Promise<void> {
        try {
            const proposalId = parseInt(req.params.id);
    ...
    private async getDashboardData(req: express.Request, res: express.Response): Promise<void> {
        try {
            const activeIds = await this.antHelper.getActiveProposals();
    ...
    private async getProposalStats(req: express.Request, res: express.Response): Promise<void> {
        try {
    ...
    private async getScorerPerformance(req: express.Request, res: express.Response): Promise<void> {
        try {
    ...
    private async getAuthorizedScorers(req: express.Request, res: express.Response): Promise<void> {
        try {
    ...
    private async authorizeScorer(req: express.Request, res: express.Response): Promise<void> {
        try {
    ...
    private getDetailedScoring(proposal: Proposal): any {
    ...
    private getProposalStatus(proposal: Proposal): string {
        if (proposal.scores.is_fulfilled) return 'fulfilled';
        if (proposal.scores.is_passing) return 'passing';
        if (proposal.scores.scorer_count > 0) return 'scored';
        if (proposal.is_active) return 'active';
        return 'inactive';
    }
    ...
    private async getScoringTrends(): Promise<any> {
